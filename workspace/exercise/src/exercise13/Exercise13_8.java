...
class BouncingBall extends Frame {
	...
	void start() {
		while (true) {
			 ...
			 * 4. y의 값이 아래 쪽 테두리(BOTTOM-BALL_SIZE)보다 작거나 같으면 
			 * 		y의 값을 BOTTOM-BALL_SIZE로 바꾸고 y축 이동방향(yStep)을 반대로 한다.
			 */
			
			//  x의 값이 왼쪽 테두리 (LEFT)보다 작거나 같으면
			if( x <= LEFT) {
				// x의 값을 LEFT로 바꾸고 x축 이동방향 (xStep)을 반대로 한다.
				x = LEFT;
				xStep *= -1;
			}
			// x의 값이 오른쪽 테두리 (RIGHT-BALL_SIZE) 보다 작거나 같으면
			if( x >= RIGHT-BALL_SIZE) {
				// x의 값을 RIGHT-BALL_SIZE로 바꾸고 x축 이동방향(xStep)을 반대로 한다 
				x = RIGHT-BALL_SIZE;
				xStep *= -1;
			}
			// y의 값이 윗 쪽 테두리 (TOP)보다 작거나 같으면
			if( y <= TOP) {
				// y의 값을 TOP으로 바꾸고 y축 이동방향(yStep)을 반대로 한다
				y = TOP;
				yStep *= -1;
