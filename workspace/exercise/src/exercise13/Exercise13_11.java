...
class BouncingBall4 extends Frame {
	...
	void start() {
		...
		while (isPlaying) {
			...
			for (int i = 0; i < size; i++) {
				...
				b.y += b.yStep;
				/*
				 * (5)아래의 로직에 맞게 코드를 작성하시오.
				 *  1. b.x 의 값이 왼쪽 테두리(LEFT)보다 작거나 같으면
				 ...
				 * 3. b.y의 값이 윗 쪽 테두리(TOP)테두리 보다 작거나 같으면 
				 * 		b.y의 값을 TOP으로 바꾸고 y축 이동방향 (b.yStep) 을 반대로 한다.
				 * 4. b.y의 값이 아래 쪽 테두리 (BOTTON-b.size) 보다 작거나 같으면
				 * 		b.y의 값을 BOTTON-b.size로 바꾸고 축 이동방향(b.yStep)을 반대로 한다.
				 */
				if( b.x <= LEFT) {
					b.x = LEFT;
					b.xStep *= -1;
	...
	boolean collisionCheck(Ball b) {
		/*
		 * (6)아래의 로직에 맞게 코드를 작성하시오.
		 *  1. 회색공의 중심과 빨간공의 중심간의 거리 (distance)가 회색공의
		 * 		반지름(BALL_SIZE/2)과 빨간공의 반지름(b.size/2)보다 같거나 작으면  
		 * 		true를 반환하고 그렇지 않으면 false를 반환한다.
		 * 
		 *		회색공의 위치가(x, y) 일 때 , 
		 *		중심 좌표는 (x+BALL_SIZE/2, y+BALL_SIZE/2)이다. 
...
class Exercise13_11 {
	public static void main(String[] args) {
		new BouncingBall4("Bouncing Ball 4").start();
	}
}
